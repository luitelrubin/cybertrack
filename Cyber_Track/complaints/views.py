...
class SocialMediaHackComplaintCreateView(generics.CreateAPIView):  # To Create Social Media Hack Complaint
    queryset = SocialMediaHackComplaint.objects.all()
    serializer_class = SocialMediaHackComplaintSerializer

class DefamationComplaintCreateView(generics.CreateAPIView):  # To Create Defamation Complaint
...
class ListAllComplaintsView(generics.ListAPIView): # To List All Complaints
        ...
        def get_queryset(self):
            ...
            combined_queryset = list(chain(financial_fraud_complaints, social_media_hack_complaints, defamation_complaints, other_complaints))
        ...
        def list(self, request, *args, **kwargs):
            ...
            if page is not None:
                financial_fraud_page = [item for item in page if isinstance(item, FinancialFraudComplaint)]
                social_media_hack_page = [item for item in page if isinstance(item, SocialMediaHackComplaint)]
                defamation_page = [item for item in page if isinstance(item, DefamationComplaint)]
                other_complaints_page = [item for item in page if isinstance(item, OtherComplaint)]
    
                financial_fraud_serializer = FinancialFraudComplaintSerializer(financial_fraud_page, many=True)
                social_media_hack_serializer = SocialMediaHackComplaintSerializer(social_media_hack_page, many=True)
                defamation_serializer = DefamationComplaintSerializer(defamation_page, many=True)
                other_complaints_serializer = OtherComplaintSerializer(other_complaints_page, many=True)
            ...
            financial_fraud_serializer = FinancialFraudComplaintSerializer(queryset, many=True)
            social_media_hack_serializer = SocialMediaHackComplaintSerializer(queryset, many=True)
...
class AllComplaintUpdateView(generics.GenericAPIView):
    ...
    def get(self, request, complaint_type, complaint_id, *args, **kwargs):
        ...
        if not queryset:
            return Response({"error": "Invalid complaint type"}, status=status.HTTP_400_BAD_REQUEST)
...
class ComplaintTrackingView(generics.GenericAPIView): # To Track Complaint
    ...
    def post(self, request, *args, **kwargs):
        ...
        try:
            complaint = queryset.get(complaint_id=complaint_id, unique_id_number=unique_id_number, date_of_birth=date_of_birth)
        except queryset.model.DoesNotExist:
            return Response({'error': 'Complaint not found or unique ID is incorrect.'}, status=status.HTTP_404_NOT_FOUND)
